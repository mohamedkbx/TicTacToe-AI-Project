...
public class TicTacToe {
	...
	public static boolean hasWon(int[][] field) {
		for (int i = 0; i < field.length; i++) {
			if (field[i][0] == field[i][1] && field[i][1] == field[i][2] && field[i][0] != 0) { //اختبار الصفوف
				return true;
			}
			if (field[0][i] == field[1][i] && field[1][i] == field[2][i] && field[0][i] != 0) { // اختبار الاعمدة
				return true;
			}
		}
	...
	private static void clearField() {
		for (int i = 0; i < field.length; i++) {
			for (int j = 0; j < field.length; j++) {
				field[i][j] = 0; //هفضي كل الصفوق والاعمدة
			}
	...
	public static boolean fieldFull(int[][] field) {
		for (int i = 0; i < field.length; i++) {  //للصفوف
			for (int j = 0; j < field.length; j++) {// للاعمدة
				if (field[i][j] == 0) { //لو في صف او عمود فاضي هيبقي ب false
					return false;
				}
			}
		}
